...
export const populateAlbum =
	(client: Client) => async (album: AlbumUpload) => {
		const artists = await Promise.all(album.artists.map(searchArtist(client)))
		return { ...album, artists } as AlbumUpload
	}

export const populateSong =
	(client: Client) => async (song: SongUpload) => {
		...
		return { ...song, album, genres, artists, remixers, featuring } as SongUpload
	}